...
3. `docs/product/AODP_ORDER_TO_CASH_WORKFLOW.md` — peta alur order-to-cash (sales order → tagihan lunas) per tahap, dengan status PASS/gap masing-masing
4. `docs/architecture/02_TECH_ARCHITECTURE.md` — arsitektur (catatan: schema real menggunakan `companies`, bukan `organizations`)
5. `docs/product/modules/*.md` — spec per modul
6. `docs/product/discovery/*.md` — business discovery & kalibrasi (Phase 3A+)
7. `docs/development/sprints/*.md` — rencana sprint
