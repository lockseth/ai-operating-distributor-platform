-- =============================================================================
-- Gate P4.02 -- payment_terms_days: sambungkan input yang hilang untuk baris
-- "Tempo" di Document Engine (PO/Invoice).
--
-- Temuan (audit template invoice vs AODP_DOCUMENT_LAYOUT_GUIDE.md LOCKED,
-- 2026-08-16): kolom sales_orders.payment_terms_days sudah ada sejak
-- migration 20260812000003, dan lapisan Document Engine sudah lengkap baca
-- kolom ini (issue_invoice_atomic menulis snapshot.paymentTermsDays,
-- print-view-model.ts menghitung dueDateLabel/"Tempo" darinya) -- TAPI tidak
-- ada jalur (UI maupun parameter RPC create/update) untuk MENGISI kolom ini
-- sama sekali. confirm_sales_order_atomic sudah punya parameter
-- p_payment_terms_days tapi orders/actions.ts hardcode null saat konfirmasi
-- (satu-satunya caller). Akibatnya baris "Tempo" selalu hilang di semua
-- invoice/PO produksi.
--
-- Scope gate ini: HANYA menambah parameter opsional baru
-- (p_payment_terms_days INTEGER DEFAULT NULL) ke create_sales_order_atomic
-- & update_sales_order_atomic, murni ADDITIVE -- pola identik migration
-- 20261005000001 (requested_delivery_date): signature lama di-DROP eksplisit
-- dulu (CREATE OR REPLACE saja akan membuat overload ambigu), baru buat
-- versi baru. TIDAK ada baris validasi/logic lain yang diubah -- body adalah
-- salinan persis dari 20261005000001, hanya titik yang menyentuh kolom baru
-- ditandai "-- BARU". CHECK constraint chk_sales_orders_payment_terms_days_positive
-- (20260812000003) TIDAK disentuh -- tetap menegakkan >0 bila diisi.
-- confirm_sales_order_atomic (20260926000001) TIDAK disentuh -- parameternya
-- sudah ada, akan tetap diisi null di confirm-time (RPC itu sendiri sudah
-- preserve nilai existing kalau p_payment_terms_days null, lihat baris
-- "payment_terms_days = CASE WHEN p_payment_terms_days IS NOT NULL THEN ...
-- ELSE payment_terms_days END" di migration itu) -- begitu diisi saat
-- create/update, nilainya otomatis terbawa sampai confirm.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. create_sales_order_atomic -- tambah p_payment_terms_days di akhir.
-- ---------------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.create_sales_order_atomic(
  UUID, UUID, TEXT, UUID, UUID, TEXT, DATE, NUMERIC, JSONB, DATE
);
...
CREATE FUNCTION public.create_sales_order_atomic(
  ...
  p_payment_terms_days INTEGER DEFAULT NULL -- BARU
)
...
-- ---------------------------------------------------------------------------
-- 2. update_sales_order_atomic -- tambah p_payment_terms_days di akhir
--    (Guard draft-only Gate 3E-D4-B2 TIDAK diubah).
-- ---------------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.update_sales_order_atomic(
  UUID, UUID, UUID, UUID, UUID, TEXT, DATE, NUMERIC, JSONB, DATE
);
...
BEGIN
  ...
  SET customer_id = p_customer_id,
      ...
      payment_terms_days = p_payment_terms_days -- BARU
  WHERE id = p_order_id;
...
REVOKE ALL ON FUNCTION public.update_sales_order_atomic(UUID, UUID, UUID, UUID, UUID, TEXT, DATE, NUMERIC, JSONB, DATE, INTEGER)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.update_sales_order_atomic(UUID, UUID, UUID, UUID, UUID, TEXT, DATE, NUMERIC, JSONB, DATE, INTEGER)
  TO service_role;
