-- =============================================================================
-- Fix: create_store_with_pic() — DUPLICATE_STORE_DETECTED tidak pernah
-- tercatat saat override.
--
-- Ditemukan via live UAT. Root cause: v_similar_id HANYA dihitung di dalam
-- blok "IF NOT p_override_similar_duplicate THEN ... END IF" -- persis blok
-- yang di-SKIP ketika override=TRUE. Akibatnya pengecekan logging di bagian
-- akhir fungsi ("IF v_similar_id IS NOT NULL AND p_override_similar_duplicate
-- THEN INSERT customer_relationship_events...") tidak pernah bernilai TRUE,
-- karena v_similar_id selalu NULL (default DECLARE) pada panggilan override.
-- Dampak: audit "similar duplicate yang di-override" (LANGKAH 4: "Admin
-- override harus menyimpan alasan dan audit") tidak pernah tersimpan.
--
-- Fix: v_similar_id SELALU dihitung terlepas dari override; RETURN awal
-- (short-circuit ke similar_duplicate_warning) hanya terjadi bila TIDAK
-- override. Migration baru (bukan edit migration yang sudah diterapkan).
-- =============================================================================
...
BEGIN
  ...
  -- FIX: v_similar_id SELALU dihitung (bukan hanya saat NOT override) supaya
  -- logging DUPLICATE_STORE_DETECTED di bawah bisa mendeteksi kondisi ini
  -- bahkan pada panggilan override=TRUE. Hanya keputusan "RETURN warning
  -- sekarang" yang bersyarat NOT override.
