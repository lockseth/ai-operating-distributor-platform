-- =============================================================================
-- Gate P4.11 -- Laporan Sales Fase B: event type baru SALES_REPORT_AFTERNOON
-- untuk Automation Outbox (migration 20260807000001). Laporan sore Owner
-- (~16:30 WIB, hari kerja) berisi hasil kerja hari itu per sales: EC-to-
-- transaksi (ORDER_COUNT), Omzet (REVENUE) vs target, dan ringkasan Tagihan
-- (piutang outstanding/overdue) -- pola sama persis dengan KPI_DAILY_SUMMARY
-- (08:00, projection pagi), channel selalu 'whatsapp', dispatch tetap
-- SELALU dry-run (lihat /api/internal/automation/dispatch, tidak berubah
-- migration ini -- keputusan dry-run itu channel-based, bukan event-based).
--
-- Scope credential baru yang dipakai: automation.sales_report_afternoon.generate
...
ALTER TABLE public.automation_outbox
  ...
  CHECK (event_type IN ('MORNING_BRIEF', 'KPI_DAILY_SUMMARY', 'SALES_REPORT_AFTERNOON'));
...
BEGIN
  ...
  IF p_event_type NOT IN ('MORNING_BRIEF', 'KPI_DAILY_SUMMARY', 'SALES_REPORT_AFTERNOON') THEN
