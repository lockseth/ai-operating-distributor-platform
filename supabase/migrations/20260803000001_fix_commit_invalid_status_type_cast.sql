-- =============================================================================
-- Fix: commit_import_batch melempar Postgres exception ("structure of query
-- does not match function result type") alih-alih mengembalikan outcome
-- invalid_status secara graceful.
--
-- Root cause: fungsi dideklarasikan RETURNS TABLE(result_outcome TEXT,
-- detail TEXT), tapi jalur invalid_status mengembalikan v_batch.status
-- (kolom import_batches.status, bertipe VARCHAR(20)) tanpa cast eksplisit.
-- Tidak seperti SELECT biasa, RETURN QUERY di PL/pgSQL menuntut kecocokan
-- TIPE PERSIS dengan signature RETURNS TABLE -- Postgres tidak melakukan
-- coercion implisit varchar->text di jalur ini, sehingga baris tersebut
-- gagal SETIAP KALI dieksekusi (bukan cuma kadang), tapi baru terdeteksi
-- sekarang karena skenario "commit batch yang statusnya masih
-- UPLOADED/MAPPED" belum pernah benar-benar dijalankan sampai ke Postgres
-- nyata sebelumnya (hanya diuji lewat mock InMemory yang tidak menegakkan
-- tipe kolom).
--
-- Ditemukan lewat Post-Commit Integration Verification Gate (TAHAP 4,
-- skenario E5) pada database Docker terisolasi baru, terpisah dari data
-- Founder mana pun.
--
-- Fix minimal: SATU baris (v_batch.status -> v_batch.status::TEXT) di
-- jalur invalid_status. Migration lama (20260801000002/000003/000004)
-- TIDAK diedit -- definisi TERBARU (000004) disalin utuh via
-- CREATE OR REPLACE FUNCTION, seluruh perilaku lain (authorization, tenant
-- isolation, idempotency, row-count parity, SECURITY DEFINER + search_path,
-- commit logic per domain) dipertahankan persis sama.
-- =============================================================================
...
BEGIN
  ...
  IF v_batch.status NOT IN ('READY_TO_COMMIT', 'FAILED') THEN
    -- FIX: cast eksplisit ke TEXT -- v_batch.status bertipe VARCHAR(20),
