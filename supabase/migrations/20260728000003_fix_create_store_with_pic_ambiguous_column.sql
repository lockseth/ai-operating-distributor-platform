-- =============================================================================
-- Fix: create_store_with_pic() — "column reference customer_id is ambiguous"
--
-- Ditemukan via smoke test langsung setelah migration diterapkan (sebelum
-- test suite/dokumentasi apa pun mereferensikan fungsi ini). Root cause:
-- RETURNS TABLE(..., customer_id UUID, ...) membuat "customer_id" menjadi
-- variabel PL/pgSQL implisit yang berlaku di seluruh body fungsi -- baris
-- "WHERE ... AND customer_id <> v_new_customer_id" (cek duplicate PIC lintas
-- toko) jadi ambigu antara kolom tabel customer_pics.customer_id dan
-- variabel OUT tsb. Fix: alias tabel eksplisit pada subquery itu.
...
BEGIN
  ...
  -- FIX: alias eksplisit "cp3" -- sebelumnya unqualified "customer_id" di
  -- WHERE clause ini ambigu dengan OUT parameter RETURNS TABLE customer_id.
  IF EXISTS (
