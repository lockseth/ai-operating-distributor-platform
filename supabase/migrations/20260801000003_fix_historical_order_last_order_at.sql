-- =============================================================================
-- Fix: commit_import_batch untuk OPEN_ORDER/HISTORICAL_ORDER membaca
-- customers.last_order_at SETELAH trigger trg_sales_orders_update_customer
-- sudah menimpanya jadi NOW() -- perbandingan "last_order_at < order_date"
-- jadi selalu FALSE (NOW() selalu lebih besar dari tanggal historis), jadi
-- koreksi TIDAK PERNAH benar-benar terjadi (customers.last_order_at diam-diam
-- tetap NOW(), bukan tanggal historis asli).
--
-- Fix: tangkap last_order_at SEBELUM INSERT (sebelum trigger sempat
-- menimpanya), lalu pakai nilai itu (bukan re-read setelah trigger) untuk
-- perbandingan GREATEST-nya.
-- =============================================================================
