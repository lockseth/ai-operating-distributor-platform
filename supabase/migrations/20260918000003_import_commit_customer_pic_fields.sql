-- =============================================================================
-- ERP Master Data Adaptation (scope reconciled -- Demo V2, import manual lewat
-- template AODP): commit_import_batch / rollback_import_batch diperluas
-- (CREATE OR REPLACE, additive) HANYA untuk:
--   - CUSTOMER_PIC: store_city/store_province (kolom baru customers.city/
--     province) + PIC opsional penuh (baris tanpa PIC hanya membuat/
--     memperbarui toko, committed_entity_table='customers', tidak membuat
--     customer_pics -- lihat validators.ts validateCustomerPicRow).
--
-- PRODUCT_PRICE TIDAK diubah sama sekali di migration ini -- kategori/rak/
-- stok/konversi kemasan (importer khusus layout ERP) di-DEFER per keputusan
-- Founder terbaru. Harga opsional + is_active dipaksa false untuk produk
-- tanpa harga TIDAK butuh perubahan SQL -- RPC asli sudah men-COALESCE
-- price/is_active dari normalizedData apa adanya (lihat
-- 20260801000002_legacy_import_commit_rollback.sql baris 161-186), logika
-- itu sepenuhnya di TypeScript (validateProductPriceRow).
--
-- PENTING -- baseline commit_import_batch yang benar untuk CREATE OR REPLACE
-- BUKAN 20260801000002, melainkan 20260803000001_fix_commit_invalid_status_
-- type_cast.sql (migration TERBARU yang menyentuh fungsi ini sebelum gate
-- ini -- diverifikasi lewat grep seluruh supabase/migrations/*.sql, tidak
-- ada yang lain di antaranya). Draft awal migration ini sempat salah
-- mem-CREATE OR REPLACE dari baseline 20260801000002 yang sudah usang,
-- yang diam-diam MEREGRESI dua fix yang sudah pernah dibuat:
--   1. v_batch.status::TEXT (20260803000001) -- tanpa cast eksplisit,
--      RETURN QUERY invalid_status melempar Postgres exception
--      "structure of query does not match function result type" (VARCHAR(20)
--      vs TEXT) SETIAP KALI dieksekusi -- ketahuan lewat integration test
--      commit-invalid-status.integration.test.ts terhadap Supabase lokal
--      sungguhan sebelum migration ini di-commit.
--   2. v_last_order_before (20260801000003) -- last_order_at HARUS ditangkap
--      SEBELUM insert sales_orders (sebelum trigger
--      trg_sales_orders_update_customer menimpanya jadi NOW()), bukan
--      re-read setelahnya.
-- Migration ini sekarang membangun dari 20260803000001 apa adanya + HANYA
-- menambahkan city/province & PIC-opsional -- tidak ada perubahan lain.
--
-- Semua kolom/behavior LAMA lainnya tetap sama persis -- ini murni
-- penambahan field pelanggan, tidak ada perubahan pada
-- OPEN_AR/OPEN_ORDER/HISTORICAL_ORDER/PRODUCT_PRICE maupun logika
-- forbidden/not_found/already_committed/invalid_status/FAILED yang sudah ada.
-- =============================================================================
...
BEGIN
  ...
  IF v_batch.status NOT IN ('READY_TO_COMMIT', 'FAILED') THEN
    -- Cast eksplisit ke TEXT wajib -- v_batch.status bertipe VARCHAR(20),
    -- RETURN QUERY menuntut kecocokan tipe persis dengan RETURNS TABLE
    -- (lihat 20260803000001_fix_commit_invalid_status_type_cast.sql).
    RETURN QUERY SELECT 'invalid_status'::TEXT, v_batch.status::TEXT;
    RETURN;
  END IF;
  ...
  BEGIN
    ...
    LOOP
      IF v_batch.import_type = 'CUSTOMER_PIC' THEN
        ...
        IF v_customer_id IS NULL THEN
          IF v_row.proposed_action = 'UPDATE' AND v_row.detected_existing_id IS NOT NULL THEN
            ...
            UPDATE public.customers SET
              ...
              city = COALESCE(NULLIF(v_row.normalized_data->>'store_city',''), city),
              province = COALESCE(NULLIF(v_row.normalized_data->>'store_province',''), province),
          ...
          ELSE
            INSERT INTO public.customers (
              company_id, code, name, phone, address, city, province, area, assigned_sales_id, is_active,
              ...
              NULLIF(v_row.normalized_data->>'store_city',''),
              NULLIF(v_row.normalized_data->>'store_province',''),
        ...
        -- PIC opsional penuh: baris tanpa data PIC (validators.ts sudah
        -- memastikan ini berarti pic_name/pic_phone/pic_roles SEMUA kosong,
        -- bukan sebagian) hanya membuat/memperbarui toko -- tidak fabricate
        -- PIC dummy. committed_entity_table diarahkan ke 'customers' supaya
        -- rollback tahu entity mana yang harus dibalik untuk baris ini.
        IF NULLIF(v_row.normalized_data->>'pic_name', '') IS NULL THEN
          UPDATE public.import_batch_rows
          SET committed_entity_id = v_customer_id, committed_entity_table = 'customers'
          WHERE id = v_row.id;
        ELSE
      ...
      ELSIF v_batch.import_type = 'PRODUCT_PRICE' THEN
        -- Tidak diubah dari 20260801000002 (kategori/rak/stok/konversi
        -- kemasan di-DEFER) -- harga opsional & is_active-force-false
        -- sepenuhnya di TypeScript, RPC ini sudah generik lewat COALESCE.
        IF v_row.proposed_action = 'UPDATE' AND v_row.detected_existing_id IS NOT NULL THEN
      ...
      ELSIF v_batch.import_type IN ('OPEN_ORDER', 'HISTORICAL_ORDER') THEN
        ...
        -- validasi).
        -- Tangkap last_order_at SEBELUM insert (sebelum trigger
        -- trg_sales_orders_update_customer sempat menimpanya jadi NOW()).
        ...
        -- Trigger sudah men-set last_order_at = NOW() -- dikoreksi memakai
        -- nilai SEBELUM insert (v_last_order_before), bukan re-read yang
        -- sudah kotor, supaya order historis tidak pernah dianggap "baru
        -- saja terjadi" (lihat 20260801000003_fix_historical_order_last_
        -- order_at.sql).
...
-- ---------------------------------------------------------------------------
-- rollback_import_batch -- tambahan: restore city/province pada UPDATE-
-- restore customers, dan handle penghapusan baris CREATE bertipe
-- committed_entity_table='customers' (toko tanpa PIC). PRODUCT_PRICE TIDAK
-- diubah dari 20260801000002.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_row.committed_entity_table = 'customers' THEN
      -- Baris toko-tanpa-PIC (kontrak PIC opsional) -- blocker sama seperti
      -- toko yang dirujuk lewat PIC: jangan hapus/restore kalau sudah punya
      -- order live di luar batch ini.
      IF EXISTS (
        SELECT 1 FROM public.sales_orders so
        WHERE so.customer_id = v_row.committed_entity_id AND so.import_batch_id IS DISTINCT FROM p_batch_id
      ) THEN
        v_blockers := v_blockers || jsonb_build_object(
          'rowNumber', v_row.row_number, 'entityTable', 'customers',
          'entityId', v_row.committed_entity_id, 'reason', 'Toko sudah punya order live di luar batch ini'
        );
      END IF;
  ...
  LOOP
    IF v_row.pre_update_snapshot IS NOT NULL THEN
      ...
      IF v_row.committed_entity_table = 'customers' THEN
        UPDATE public.customers SET
          ...
          address = v_row.pre_update_snapshot->>'address',
          city = v_row.pre_update_snapshot->>'city', province = v_row.pre_update_snapshot->>'province',
          area = v_row.pre_update_snapshot->>'area',
    ...
    ELSE
      ...
      ELSIF v_row.committed_entity_table = 'customers' THEN
        -- Baris toko-tanpa-PIC yang dibuat batch ini (bukan UPDATE, karena
        -- pre_update_snapshot NULL di sini) -- aman dihapus langsung, sama
        -- syaratnya dengan cabang customer_pics di atas.
        DELETE FROM public.customers c
        WHERE c.id = v_row.committed_entity_id
          AND c.import_batch_id = p_batch_id
          AND NOT EXISTS (SELECT 1 FROM public.customer_pics cp2 WHERE cp2.customer_id = c.id);
