...
1. `[REQUEST FOUNDER]` **WhatsApp Bablast — Gate P4.13 kirim nyata SELESAI
   (2026-08-22, lokal), penjadwal otomatis BELUM ADA.** Keputusan arsitektur
   sudah final (2026-08-22): panggil Bablast **langsung dari server AODP**,
   TANPA n8n. Dibangun: client `lib/integrations/bablast.ts` (send/status/
   pairing), dispatch route kirim nyata via saklar terpisah
   `BABLAST_DRY_RUN` (default aman, sama pola `AUTOMATION_DRY_RUN`), kartu
   "Koneksi WhatsApp (Bablast)" di `/dashboard/automation` (Owner tinggal
   klik "Mulai Pairing" → scan QR/kode di HP nomor pengirim, tanpa langkah
   teknis lain), 3 route generator laporan (`kpi-daily-summary`,
   `sales-report-afternoon`, `collection-plan-morning`) diubah kirim ke
   nomor telepon Owner asli (`users.phone`, dinormalisasi ke format
   `62xxx`) bukan lagi placeholder `owner:<id>`.
   **Gap yang tersisa — TIDAK ADA yang memicu 3 laporan itu otomatis lagi**:
   file n8n (`aodp-kpi-daily-summary.json` dkk) sudah tidak relevan karena
   keputusan "tanpa n8n" — dan Vercel project ini **plan Hobby** (Cron Job
   dibatasi 1x/hari, tidak bisa poll tiap menit seperti dispatcher lama).
   Rencana penggantinya: Vercel Cron native (gratis di Hobby) + digabung
   generate+kirim jadi satu langkah per laporan (bukan lagi generate-lalu-
   dispatcher-terpisah) — **belum dieksekusi**, perlu 1 credential internal
   baru untuk cron (tidak perlu Bearer token n8n) + `vercel.json`. Juga
   perlu: (a) pairing nomor `+6285185905859` di production (belum pernah
   dicoba nyata), (b) Founder set `BABLAST_DRY_RUN=false` di Vercel setelah
   yakin siap (sengaja tidak otomatis nyala saat deploy).
