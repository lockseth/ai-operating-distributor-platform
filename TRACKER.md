...
| Branch | `main` |
| HEAD | `58118e5` — **sinkron penuh dengan `origin/main`** (0 ahead/behind). Mencakup Gate P4.15 (Morning Brief WhatsApp) **terverifikasi hosted penuh** (lihat Log Milestone "Gate P4.15 susulan") plus fix format Rupiah di semua laporan KPI, dan **Rencana Penagihan sekarang juga dikirim personal ke tiap sales** (bukan cuma rekap Owner) — **terverifikasi hosted PENUH termasuk fisik 2 HP terpisah dikonfirmasi Founder** ("oke sudah masuk mantap"), plus retry otomatis dari error transient Bablast (HTTP 525). Migration terbaru `20261015000001` (bersihkan `last_error` saat job sukses). `BABLAST_DRY_RUN=false` aktif, pagar `BABLAST_TEST_OVERRIDE_PHONE` **aktif kembali** (nilai nomor tes istri Founder) setelah dilepas-pasang 2x dengan izin eksplisit Founder tiap kali untuk uji routing per-salesman ke 2 nomor tes berbeda. Nomor demo Salma/Waluyo(sales) sekarang pakai nomor tes aman Founder sendiri, bukan lagi berbagi nomor asli Pak Waluyo. |
...
|---|---|---|---|
| Telegram Sales Order Entry (input order sales) pindah ke WhatsApp juga? | `[REQUEST FOUNDER]` | 2026-08-22 | Dilontarkan Founder saat Gate P4.15 (Morning Brief pindah ke WA), CTO rekomendasikan **skip dulu**, Founder setuju. Beda kelas risiko dari laporan: order entry itu bot interaktif (menu bertingkat, state), belum ada bukti Bablast (Unofficial Sender) mendukung pesan masuk/tombol interaktif sama sekali — perlu riset kemampuan Bablast dulu secara terpisah sebelum dipertimbangkan lagi, bukan keputusan yang bisa diambil buru-buru. |
...
| **Lock toko yang punya tagihan tertunggak** (blokir order baru sampai lunas) + alur pengajuan buka-lock oleh sales dengan approval Owner (notifikasi real-time) — insight Pak Waluyo | `[REQUEST FOUNDER]` | 2026-08-19 | Scope besar & masih ambigu di beberapa titik, sengaja BELUM dieksekusi, perlu klarifikasi CTO dulu: (a) ambang "tertunggak" — overdue berapa hari, atau setiap ada outstanding sedikit pun? (b) "locked" itu blokir SEMUA order baru atau cuma order kredit (cash tetap boleh)? (c) kalimat "pengajuan open locking pun hanya jika toko melakukan order dibayar lunas" ambigu — apakah maksudnya sales HANYA BOLEH ajukan buka-lock kalau sudah ada order baru yang lunas duluan (bukti), atau unlock terjadi OTOMATIS begitu ada order lunas (bukan pengajuan manual)? (d) "notifikasi real-time" ke Owner — kalau mau lewat WA, jalur ini sama dengan gap Owner Approval Inbox WA di baris di bawah (Gate P4.08, Backlog #9 Jalur A masih terbuka) — perlu diputuskan pakai in-app inbox dulu (sudah ada pola-nya, sudah jalan) sambil nunggu WA, atau bangun `call_bablast` dulu baru fitur ini. Ini juga menyentuh alur create/confirm order (gate LOCKED lain) — perlu direncanakan hati-hati, bukan sekadar tambahan kecil. |
| **Notifikasi WhatsApp real-time saat sales ajukan harga khusus** (Gate P4.08 "Jalur A") — bangun action `call_bablast` di automation engine, langsung tanpa n8n? | `[TEMUAN]` | 2026-08-22 (diaudit ulang, ditemukan asli 2026-08-19) | Detail lengkap: Backlog #9. Sekarang jadi satu-satunya bagian sistem yang masih bergantung n8n (Jalur B/laporan terjadwal sudah lepas total dari n8n sejak Gate P4.13-15). Dampak nyata RENDAH — Owner Approval Inbox in-app sudah jalan penuh, ini cuma notifikasi tambahan. CTO tidak membangun ini tanpa arahan eksplisit karena ini penambahan scope fitur (keputusan produk), bukan bug fix — perlu keputusan Founder apakah prioritas. |
...
9. **[TEMUAN 2026-08-19, DIAUDIT ULANG 2026-08-22]** Audit arsitektur "event AODP → WhatsApp" — awalnya ada **2 pipeline terpisah**, keduanya belum siap kirim WA nyata. **Status sekarang jauh beda antara keduanya**:
   ...
   - **Jalur A (event-triggered — dipakai Gate P4.08, notifikasi WA saat sales ajukan harga khusus) — MASIH TERBUKA, arsitektur tidak berubah sejak 2026-08-19.** Dicek ulang 2026-08-22: `automation_rules` DAN `n8n_webhooks` tenant ini **masih 0 baris** — `submitSpecialPriceProposalAction` memanggil `processAutomationEvent`, tidak menemukan rule apa pun, jadi benar-benar no-op diam-diam (bukan cuma "skipped", malah tidak ada log sama sekali karena 0 rule untuk di-loop). **Dampak nyata rendah**: notifikasi WA ini SENGAJA didesain "best-effort" di atas Owner Approval Inbox in-app yang sudah jalan penuh (lihat komentar `special-price-proposal-actions.ts`) — pengajuan harga khusus tidak pernah terganggu, Owner tetap lihat semua pengajuan di `/dashboard/orders/approvals`, cuma tidak dapat ping WA tambahan. **Temuan arsitektur baru**: karena Jalur B sekarang panggil Bablast LANGSUNG tanpa n8n, Jalur A jadi satu-satunya bagian sistem yang masih bergantung ke n8n sebagai perantara — tidak konsisten lagi dengan pola yang sudah terbukti jalan. **Rekomendasi CTO kalau Founder mau notifikasi real-time ini dinyalakan**: bukan lanjutkan jalur n8n lama (perlu 2 pekerjaan: workflow n8n nyata + koneksi ke Bablast, masih belum tersambung sama sekali), tapi tambah action type baru `call_bablast` di `lib/automation/engine.ts` (paralel dengan `call_n8n` yang sudah ada) yang panggil `sendBablastMessage()` langsung — konsisten dengan arsitektur Jalur B, tanpa n8n. **Belum dikerjakan** karena ini penambahan scope fitur (keputusan produk), bukan cuma bug fix — menunggu keputusan Founder apakah notifikasi real-time approval ini prioritas atau tetap cukup in-app saja untuk sekarang.
...
|---|---|---|---|
| 2026-08-22 | `[REQUEST FOUNDER]` **Rencana Penagihan juga dikirim personal ke tiap sales, bukan cuma rekap Owner** | Founder amati: "Rencana penagihan juga harus nya di tampilan di sales" -- sebelumnya cuma satu pesan rekap-semua-sales ke Owner (Gate P4.12), sales sendiri tidak pernah tahu toko mana yang perlu DIA tagih hari itu lewat WhatsApp. Dibangun `buildCollectionPlanForSalesman()` (versi personal, "Selamat pagi, {nama}" + daftar toko miliknya saja, `lib/n8n-automation/collection-plan-morning.ts`) + `collectionPlanForSalesmanIdempotencyKey` (namespace terpisah dari key Owner). Route generator sekarang enqueue 1 job tambahan per sales aktif+ada nomor WA (reuse `listEligibleWhatsAppRecipients`, pola sama Morning Brief Gate P4.15) -- rekap Owner yang sudah ada **tidak diubah**, ini tambahan bukan pengganti. Selalu kirim walau 0 toko (konsisten Morning Brief) supaya sales tahu laporan jalan normal. Diverifikasi: +4 test builder, 185 test n8n-automation gabungan PASS, `tsc` bersih, **end-to-end nyata hosted**: trigger cron production → 2 job baru masuk outbox dengan `recipient_reference` benar per-orang (nomor tes aman masing-masing) dan teks personal benar. Satu kirim (Salma) sempat kena **error transient provider** (`Bablast /send gagal (525)` -- Cloudflare SSL handshake failed sisi Bablast, bukan bug kode kita) -- otomatis masuk `RETRY`, dicoba ulang manual setelah `available_at` lewat → **SENT bersih**, `last_error` kosong (membuktikan migration `20261015000001` di atas juga bekerja benar di kasus nyata). **Catatan verifikasi fisik**: percobaan pertama pagar `BABLAST_TEST_OVERRIDE_PHONE` lupa dilepas dulu (CTO lalai) -- `recipient_reference` di DB sudah benar berbeda per orang, tapi pengiriman AKTUAL tetap dipaksa override ke satu nomor (Founder laporkan kedua WA masuk ke nomor istri, bukan 2 nomor terpisah). Diperbaiki: 2 baris outbox lama dihapus, override dilepas lagi (izin eksplisit Founder ke-2 kalinya), di-generate+kirim ulang → **DB DAN fisik kedua HP dikonfirmasi Founder benar** ("oke sudah masuk mantap") -- Waluyo di `081220065534`, Salma di nomor istri, masing-masing pesan sendiri-sendiri. Override dipasang kembali + redeploy segera setelahnya, `/api/health` healthy. **Pelajaran**: bukti `recipient_reference` di database SAJA tidak cukup kalau pagar override masih aktif -- verifikasi hosted WhatsApp wajib cek fisik HP penerima, bukan cuma query DB. | **PASS — hosted, termasuk jalur retry nyata DAN verifikasi fisik 2 HP terpisah oleh Founder.** |
| 2026-08-22 | `[TEMUAN]` **Gate P4.15 susulan — verifikasi hosted nyata: sender Bablast terputus, bug parsing QR, format uang mentah di semua laporan, data demo 4 akun berbagi 1 nomor** | Verifikasi hosted Gate P4.15 (baris di bawah) pakai nomor tes aman (istri Founder) menemukan Bablast sender **disconnect dari sesi sebelumnya** ("Anda harus pairing dulu") -- bukan bug kode, sesi WhatsApp Web-style memang bisa putus sendiri. Saat re-pairing, ketemu **bug baru**: kartu "Mulai Pairing" kosong lagi -- kontrak nyata `POST /connector/pairing` mode QR ternyata `data` adalah **STRING mentah** (`"https://wa.me/settings/linked_devices#2@..."`), bukan object `{qrCode}` seperti dugaan sebelumnya (kontrak Bablast tidak konsisten antar endpoint/mode). Fix: `initiateBablastPairing` (`lib/integrations/bablast.ts`) tangani kasus `data` string, kartu pairing di-encode jadi gambar QR **di browser** pakai package baru `qrcode` (bukan API generator pihak ketiga -- string ini setara token linking sesi WA, sensitif). +2 test regresi. Setelah re-pairing sukses, tes kirim Morning Brief nyata ke nomor aman **berhasil**, tapi review isi pesan (screenshot WA Founder) menemukan Revenue/Omzet/Tagihan tampil angka mentah tanpa format Rupiah, dan wording "EC Rate: 100% (insight, bukan KPI)" berpotensi menyesatkan untuk sampel kecil (Call 1x). Founder pilih **perbaiki sekarang**: `formatCurrencyLine()` baru (reuse `formatRupiah()` yang sudah ada di `document-engine/monetary.ts`, tidak duplikasi), diterapkan ke Revenue (Morning Brief), Omzet+Tagihan (Laporan Sore), outstanding+janji bayar (Rencana Penagihan); EC Rate sekarang tampilkan pecahan call mentah juga ("50% dari 2/4 call") di Morning Brief, Tutup Hari, DAN Telegram `/start` (konsisten, wording sama dipakai 3 tempat). **Migration `20261015000001`**: `complete_automation_job()` sekarang bersihkan `last_error` saat job akhirnya SENT (ditemukan job yang sukses di retry ke-2 tapi tetap tampilkan pesan gagal lama, menyesatkan pembaca outbox). **Temuan data terpisah**: query `user_roles` tenant ini mengungkap **owner + admin + 2 salesman demo (Salma, Waluyo) semuanya pakai nomor yang SAMA** (`087778404085`, nomor asli Pak Waluyo) -- inilah kenapa pagar `BABLAST_TEST_OVERRIDE_PHONE` sangat krusial (tanpa itu SEMUA laporan otomatis untuk SEMUA user bakal kirim ke nomor Pak Waluyo). Atas instruksi Founder, nomor 2 akun sales demo diganti ke nomor tes aman milik Founder sendiri (Salma → nomor istri, Waluyo-sales → nomor kedua) -- akun Owner asli TIDAK disentuh. Untuk membuktikan routing per-akun benar (bukan cuma override yang menyamakan semua ke 1 nomor), override **dilepas sementara dengan izin eksplisit Founder** ("oke lepas") -- sistem sempat memblokir aksi ini otomatis karena persis pagar hasil insiden sebelumnya, dilanjutkan hanya setelah konfirmasi langsung. Trigger ulang: 2 pesan `SENT`, `last_error: null`, `recipient_reference` benar berbeda per akun (`6281220065534` vs `6283823034645`). **Override dipasang kembali segera setelahnya** (nilai nomor istri Founder) + redeploy, dikonfirmasi via `vercel env ls`. Diverifikasi: seluruh suite lib terkait PASS (2 kegagalan pre-existing tidak terkait, path scratchpad session lain), `tsc` bersih file disentuh, migration diterapkan hosted (`supabase db push --linked`, dikonfirmasi `schema_migrations`). | **PASS — hosted, dibuktikan end-to-end nyata dengan nomor tes aman.** |
| 2026-08-22 | `[REQUEST FOUNDER]` **Gate P4.15 — Morning Brief pindah channel Telegram ke WhatsApp** | Founder minta pindah alasan: (1) historis Telegram dipilih karena provider WA lama (Fonnte) mahal, sekarang Bablast lebih murah & sudah terhubung; (2) sales lebih familiar WhatsApp; (3) WA AODP tidak dipakai chatbot customer jadi tidak ada risiko campur channel. **Sempat dibahas juga usul pindahkan Telegram Sales Order Entry (input order) ke WA sekalian — CTO rekomendasikan TIDAK, dan Founder setuju skip untuk saat ini**: order entry itu bot interaktif penuh (menu bertingkat, state), belum ada bukti Bablast (Unofficial Sender) mendukung pesan masuk/tombol interaktif, dan taruhannya jauh lebih besar (order entry berhenti = sales tidak bisa transaksi, beda kelas risiko dari laporan yang cuma informasional). Dibangun: method baru `listEligibleWhatsAppRecipients` di `salesman-directory.ts` (salesman aktif + nomor telepon valid, dinormalisasi `62xxx`) — method Telegram lama (`listEligibleMorningBriefRecipients`) **tetap dipakai apa adanya** oleh 3 laporan Owner lain (KPI/Sore/Penagihan) untuk daftar baris salesman, sama sekali tidak terpengaruh. Route `morning-brief` ganti `channel: "telegram"` → `"whatsapp"`, `recipientReference` dari `telegramChatId` → nomor telepon. Diverifikasi: 203 test PASS (+5 test `salesman-directory.test.ts`), `tsc` bersih, **end-to-end nyata lokal**: 3 sales lokal (nomor format `0812-0000-000X` dgn dash) → `curl` ke endpoint → payload benar → outbox 3 baris `channel: whatsapp`, nomor ternormalisasi bersih (`6281200000005` dst, dash terbuang otomatis). Data uji dibersihkan. Telegram Sales Order Entry TIDAK disentuh sama sekali. | **PASS — lokal + hosted.** |
| 2026-08-22 | `[TEMUAN]` **Insiden kepercayaan client — trigger tes kirim WA nyata ke nomor Owner (Pak Waluyo) tanpa pemberitahuan dulu, + 2 fix susulan** | Setelah pairing Bablast sukses (nomor sender `6285287539900`, digabung sementara dengan ASOS untuk testing), CTO trigger manual `collection-plan-morning` sebagai "tes kirim pertama" atas persetujuan Founder — **tapi TIDAK memberi tahu dulu bahwa penerimanya adalah nomor Owner tenant asli (`6287778404085`, Pak Waluyo)**, bukan nomor Founder sendiri. Founder menegur keras ("ngaco kamu") — sah, karena ini kirim data bisnis sungguhan ke client dari nomor asing tanpa konteks, berisiko dikira spam/disadap. **Root cause**: sistem laporan (P4.11-P4.13) memang didesain "kirim ke Owner terdaftar", tidak ada jalur kode yang membedakan "ini tes" dari "ini beneran" — trigger manual otomatis ikut memakai data produksi apa adanya. **Tindakan langsung**: (1) `BABLAST_DRY_RUN` dikembalikan ke `true` (pause total kirim nyata) sampai ada kejelasan; (2) dibangun **pagar keras baru**: `BABLAST_TEST_OVERRIDE_PHONE`/`TELEGRAM_TEST_OVERRIDE_CHAT_ID` — kalau diset, SEMUA pengiriman channel itu (termasuk saat dry-run dimatikan) otomatis dialihkan ke nomor/chat id override, `recipient_reference` produksi asli diabaikan total. Modul baru `lib/n8n-automation/dispatch-target.ts` (+5 test) dipakai `/dispatch` route. Ini pagar di level kode, bukan sekadar checklist "inget-inget sebelum trigger" yang terbukti gagal hari ini. **Resolusi**: Founder cek langsung HP Pak Waluyo — pesan tes (`Rencana Penagihan 2026-08-22`) terbukti masuk normal, konten benar, terbaca, **tidak ada indikasi client curiga/terganggu**. Founder konfirmasi lanjut, `BABLAST_DRY_RUN` dikembalikan ke `false`, `/api/health` healthy pasca redeploy. Pagar `BABLAST_TEST_OVERRIDE_PHONE`/`TELEGRAM_TEST_OVERRIDE_CHAT_ID` **tetap ada di kode** (tidak aktif kalau env var tidak diset -- tidak mengubah perilaku produksi normal) sebagai jaring pengaman kalau ada trigger tes lagi ke depan. | **PASS — kirim nyata aktif lagi, insiden ditutup, pagar tetap terpasang untuk tes berikutnya.** |
