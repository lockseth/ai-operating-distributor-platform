...
| Branch | `main` |
| HEAD | `4500211` — **sinkron penuh dengan `origin/main`** (0 ahead/behind). Mencakup seluruh gate s.d. Gate P4.14 (Vercel Cron menggantikan n8n sebagai penjadwal untuk 4 laporan terjadwal) dan Gate P4.13 (Bablast kirim nyata + pairing UI). Migration s.d. `20261014000001` (Gate P4.12) SUDAH di-`db push` — P4.13/P4.14 TIDAK menambah migration baru (murni kode + 1 baris credential operasional). `CRON_SECRET`/`INTERNAL_AUTOMATION_TOKEN` sudah di-set Vercel Production. |
