...
| Branch | `main` |
| HEAD | `7c6b396` — **sinkron penuh dengan `origin/main`** (0 ahead/behind). Mencakup Gate P4.15 (Morning Brief WhatsApp) **terverifikasi hosted penuh** hari ini (lihat Log Milestone "Gate P4.15 susulan") plus fix format Rupiah di semua laporan KPI. Migration terbaru `20261015000001` (bersihkan `last_error` saat job sukses). `BABLAST_DRY_RUN=false` aktif, pagar `BABLAST_TEST_OVERRIDE_PHONE` **aktif kembali** (nilai nomor tes istri Founder) setelah sempat dilepas sebentar (dengan izin eksplisit Founder) untuk uji routing per-salesman ke 2 nomor tes berbeda. |
