...
| Notifikasi WhatsApp realtime ke Owner (Owner Approval Inbox) belum benar-benar kirim — provider Bablast dipilih Founder tapi API key/endpoint/template WA belum disetujui Meta | `[REQUEST FOUNDER]` | 2026-08-18, **update 2026-08-20** | Sisi AODP (trigger event + payload pesan + link) sudah selesai & teruji (Gate P4.08). **KOREKSI (2026-08-19, audit arsitektur lengkap — lihat Backlog #9)**: klaim "tinggal daftarkan `n8n_webhooks`, tidak perlu kode baru" **TERNYATA TIDAK LENGKAP** — 2 pekerjaan KODE tambahan tetap perlu (workflow n8n belum tersambung ke Bablast, tidak ada UI daftar `n8n_webhooks`). **Konfirmasi 2026-08-20**: CTO sudah cek — Bablast TIDAK menimbulkan lock-in arsitektur, aman dipakai. **UPDATE 2026-08-20**: `BABLAST_API_KEY` **SUDAH DIKONFIGURASI** Founder sendiri di Vercel Environment Variables (scope Production) — dicek CTO, nilainya tidak pernah diketik/dilihat CTO (kebijakan keamanan: API key tidak boleh masuk chat/dokumen/commit). **Masih menunggu dari Founder**: (a) endpoint API "Send Message" Bablast (URL-nya, bukan cuma key) + format request body-nya (field nomor tujuan, isi pesan, dan apakah butuh parameter template WA — biasanya wajib utk WhatsApp Business API resmi), (b) template pesan yang sudah disetujui Meta. Begitu itu ada, CTO lanjut bangun workflow n8n → Bablast beneran (Backlog #9 item 2). |
